...
var Sync = function (localpath, remotepath) {
    var that = this;
...
Sync.create = function (localpath, remotepath) {
    return new Sync(localpath, remotepath);
};
...
Sync.prototype.updateTime = function () {
    ...
    console.log(('updating local mtime: ' + localpath).green);
    ftp.ls(remotepath, function(err, fileArray) {
        //control.ftp.keepAlive();
        if (err) {
            console.log(err);
        }
        console.log(('setting local mtime: ' + localpath).green);
...
proto.sync = function (req, res) {
    ...
    var update = false,
        ...
        connectCallback = function () {
            var remotepath = control.config.root + path.sep + req.params[0],
            ...
            ftp.chdir(control.config.root, function (err, success) {
                if (err) {
                    ...
                    Sync.create(localpath, remotepath);
                }
            });
        };
