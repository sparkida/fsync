...
var Sync = function (localpath, remotepath, req, res) {
    var that = this;
    that.req = req;
    that.res = res;
    that.localpath = localpath;
    that.remotepath = remotepath;
	console.log('Syncing localpath: %s', localpath);
...
Sync.create = function (localpath, remotepath, req, res) {
    return new Sync(localpath, remotepath, req, res);
};
...
Sync.prototype.checkDir = function () {
                ...
                var bindFunc = function (index) {
					//console.log(index, filelist[index]);
                        main.cue.push(function () {
                            ...
                            Sync.create(path.join(localpath, filelist[index]), path.join(remotepath, filelist[index]), that.req, that.res);
                        });
                    };
    ...
    ftp.mkdir(remotepath, function (err, dir) {
        ...
        connectCallback = function () {
            ...
            ftp.chdir(config.root, function (err, success) {
                if (err) {
                    ...
                    Sync.create(localpath, remotepath, req, res);
                }
            });
        };
